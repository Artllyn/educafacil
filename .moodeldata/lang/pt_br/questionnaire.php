...
$string['downloadtextformat_help'] = 'Este recurso permite salvar todas as respostas de uma enquete em um formato de arquivo compatível e selecionável.
Você pode optar por incluir itens de dados extras na exportação, bem como enviar automaticamente o arquivo para usuários selecionados.';
...
$string['feedbackoptions_help'] = 'Opções de feedback estão disponíveis se a sua enquete contém os seguintes tipos de questões e configurações:
Botões de rádio; Menu Dropdown ou Avaliação. Estas questões devem ser definidas como obrigatórias, o campo Nome da Questão não deve estar vazio e as opções de possíveis respostas devem conter um valor.';
...
$string['submissionnotificationhtmluser'] = '<a href="{$a->profileurl} ">{$a->username}</a> tem um novo <a href="{$a->submiturl}">envio</a> para a enquete "{$a->name}" no curso "{$a->coursename}".';
...
$string['submissionnotificationtextuser'] = 'coursename{$a->username} ({$a->profileurl}) tem um novo envio ({$a->submiturl}) para a enquete "{$a->name}" no curso "{$a->coursename}".';
